...
class BO_optimization_sto(BO_optimization):
    ...
    def get_next_points(self, init_x, init_y, best_init_y, n_points = 1):
        init_x, init_y, bounds_torch, x_max, x_min = self.prepare_data_input(init_x, init_y, best_init_y)
        ...
        pred_model, pred_param = train_model(key= ensemble_key[0], train_method = "ntkgp_param", 
                            train_data = train_data, test_data=None, \
                            parameterization='ntk', \
                            activation='relu', W_std= 1.5, \
                            b_std = 0.05, width = 16, depth = 2, \
                            learning_rate = 1e-3, \
                            training_steps = 10000, noise_scale = 0.05)

        x_obtained = acq_max(ac=self.util_ts.utility, pred_model=pred_model, pred_param=pred_param, bounds = bounds)
