...
def obtain_bo_other(bo_type):
    ...
    # 模型导入========================================================================
    elif bo_type == 'expllmopt':
        from bo_models.expllmopt import expllmopt as bo_used
    return bo_used

# Main loop for running Bayesian optimization
def bo_loop(bo_type, n_repetitions, fun_to_evaluate, config_space, order_list, n_runs, n_init, list_init_config, task_name=None, model_name=None, metric_name=None):
    all_final_y    = []
    all_metrics_pd = []
    for idx in range(n_repetitions):
        # Different handling based on the type of Bayesian optimization
        # 添加新opt
        if bo_type in ['bo_hebo', 'bo_turbo', 'bo_tpe', 'bo_smac', 'bo_optuna', 'bo_skopt']:
        ...
        elif bo_type in ['bo_gp','bo_dkl','bo_random','bo_gp_turbo','bo_dngo','bo_sto']:
        ...
        # if bo_type in ['expllmopt']:
        #     bo_used = obtain_bo_other(bo_type)
        #     final_y, all_metrics = bo_used(fun_to_evaluate, config_space, n_runs = n_runs, n_init = n_init,
        #                                     seed = idx, task_name=task_name, model_name=model_name, metric_name=metric_name,config_init = list_init_config[idx], order_list = order_list)
...
def run_all_models(fun_to_evaluate, all_dict_templates, n_repetitions, n_runs, n_init, config_space, order_list, path_name, task_name=None, model_name=None, metric_name=None):
        ...
        for this_metric in list_metric:
            for this_loader in list_data:
                for this_model in list_model:
                    ...
                    run_all_models(fun_to_evaluate, all_dict_templates, n_repetitions, n_runs, n_init, config_space, order_list, path_name, task_name=this_loader, model_name=this_model,metric_name=this_metric)
...
def run_tabular(list_model, list_data, all_dict_templates, n_repetitions, n_runs, n_init):
        ...
        for data_id in list_data:
            ...
            for this_model in list_model:
                ...
                # run_all_models(fun_to_evaluate, all_dict_templates, n_repetitions, n_runs, n_init, config_space, order_list, path_name)
                run_all_models(fun_to_evaluate, all_dict_templates, n_repetitions, n_runs, n_init, config_space, order_list, path_name, task_name=data_id, model_name=this_model,metric_name="objective_score")